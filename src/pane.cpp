...
{
    if (const auto native_handle = image.get_native_handle(); native_handle) {
        ImGui::Image(reinterpret_cast<void*>(static_cast<intptr_t>(*native_handle)), { width.get(), height.get() });
    }
}
